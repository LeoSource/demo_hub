...
void test_joint_traj()
{
	int nj = 5;
	Eigen::VectorXd waypoint(nj);
	std::list<Eigen::VectorXd> waypoints;

	waypoint<<0,0,0,0,0;
	waypoints.push_back(waypoint);
	waypoint<<0.02,0.03,30,40,0.03;
	waypoints.push_back(waypoint);

	Eigen::VectorXd vmax(nj);
	vmax<<0.005,0.005,20,20,0.005;
	Eigen::VectorXd amax(nj);
	amax<<0.01,0.01,30,30,0.01;

	Trajectory trajectory(Path(waypoints, 0.002), vmax, amax, 0.005);
	trajectory.outputPhasePlaneTrajectory();
	if(trajectory.isValid())
	{
